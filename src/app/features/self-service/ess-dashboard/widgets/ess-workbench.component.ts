  ...
  template: `
    <div class="flex h-full flex-col overflow-hidden rounded-md bg-white shadow-sm ring-1 ring-slate-100 transition-all hover:shadow-2xl hover:shadow-slate-200/40">
      <div class="flex flex-col shrink-0 gap-6 p-8 border-b border-slate-50 bg-slate-50/20 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div class="flex items-center rounded-md bg-white p-1.5 shadow-sm ring-1 ring-slate-100">
          <button
            type="button"
            ...
            class="rounded-md px-6 py-2 text-[11px] font-black uppercase tracking-widest transition-all"
          >
      ...
      <div class="grid grid-cols-2 gap-4 border-b border-slate-50 p-8">
        <div class="rounded-md bg-slate-50 p-6 ring-1 ring-slate-100 transition-all hover:shadow-md">
        ...
        <div class="rounded-md bg-emerald-50 p-6 ring-1 ring-emerald-100/50 transition-all hover:shadow-md">
          ...
          <div class="space-y-4">
            @for (project of projects(); track project.id) {
              <button
                ...
                class="group flex w-full flex-col rounded-md bg-slate-50 p-6 transition-all hover:bg-white hover:shadow-xl hover:shadow-slate-200/40 ring-1 ring-transparent hover:ring-slate-100"
              >
                <div class="flex w-full items-start justify-between">
                  ...
                  <span class="rounded-md bg-white px-4 py-1.5 text-[11px] font-black text-emerald-600 shadow-sm ring-1 ring-slate-100">
                    {{ project.progress }}%
                  </span>
                </div>
            ...
            @if (projects().length === 0) {
              <div class="flex flex-col items-center justify-center p-10 text-center">
                 ...
                 <button (click)="navigate.emit('/projects')" class="mt-6 rounded-md bg-slate-900 px-8 py-3.5 text-[11px] font-black uppercase tracking-widest text-white hover:bg-emerald-600 transition-all">
          ...
          <div class="space-y-4">
            @for (timesheet of timesheets(); track timesheet.id) {
              <button
                ...
                class="group flex w-full items-start justify-between rounded-md bg-slate-50 p-6 transition-all hover:bg-white hover:shadow-xl hover:shadow-slate-200/40 ring-1 ring-transparent hover:ring-slate-100"
              >
                <div class="min-w-0 flex-1 pr-6">
                  ...
                  <div class="mt-4 flex flex-wrap gap-4 text-[10px] font-bold uppercase tracking-widest text-slate-400">
                    ...
                    <span class="px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-600">{{ timesheet.status }}</span>
                  </div>
                </div>
            ...
            @if (timesheets().length === 0) {
              <div class="flex flex-col items-center justify-center p-10 text-center">
                 ...
                 <button (click)="navigate.emit('/timesheets')" class="mt-6 rounded-md bg-slate-900 px-8 py-3.5 text-[11px] font-black uppercase tracking-widest text-white hover:bg-emerald-600 transition-all">
