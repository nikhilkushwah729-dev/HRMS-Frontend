  ...
  template: `
    <div class="app-glass-card h-full flex flex-col ring-1 ring-slate-100 hover:ring-indigo-100 transition-all shadow-sm hover:shadow-2xl overflow-hidden rounded-md">
      <div class="p-5 border-b border-slate-100 bg-slate-50/30 flex items-center gap-4">
        ...
        <div>
          <h2 class="text-lg font-black text-slate-900 tracking-tight leading-none">{{ t('selfService.holidaysWidget.title') }}</h2>
          <p class="text-[10px] font-medium text-slate-500 mt-1.5 uppercase tracking-wider opacity-70">{{ t('selfService.holidaysWidget.subtitle') }}</p>
        </div>
      </div>
      <div class="flex-1 p-4 flex flex-col gap-2 overflow-y-auto custom-scrollbar">
        @for (holiday of holidays(); track holiday.date) {
          <div class="flex items-center justify-between rounded-md border border-slate-100 bg-white p-4 group hover:border-indigo-100 transition-colors shadow-sm">
            <div class="flex items-center gap-3 min-w-0">
              <div class="h-9 w-9 flex-shrink-0 flex items-center justify-center rounded-md bg-sky-50 text-sky-600 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors">
        ...
        @if (holidays().length === 0) {
          <div class="flex-1 flex flex-col items-center justify-center p-8 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest opacity-60">
             <p>{{ t('selfService.holidaysWidget.empty') }}</p>
          </div>
        }
      </div>
    </div>
