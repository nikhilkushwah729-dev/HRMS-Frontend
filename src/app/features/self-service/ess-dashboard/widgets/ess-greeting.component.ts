    ...
    <div class="grid grid-cols-12 gap-6">
      ...
      @if (specialMessage().length > 0) {
        <div class="col-span-12 animate-fade-in group">
          <div class="relative overflow-hidden rounded-[40px] bg-gradient-to-r from-indigo-600 via-indigo-700 to-purple-700 p-8 shadow-2xl transition-all hover:shadow-indigo-200/50">
            <div class="relative z-10 flex items-center justify-between">
              <div class="flex items-center gap-8">
                <div class="flex h-20 w-20 items-center justify-center rounded-[32px] bg-white/20 text-white backdrop-blur-xl ring-1 ring-white/30 group-hover:scale-110 transition-transform">
              ...
              <button (click)="closeBanner.emit()" class="flex h-12 w-12 items-center justify-center rounded-[20px] bg-white/10 text-white transition-all hover:bg-white/20 hover:rotate-90">
      ...
      <div class="col-span-12">
        <div class="app-surface-card group flex flex-col gap-6 p-6 md:flex-row md:items-center md:justify-between sm:p-8">
          <div class="flex items-center gap-6">
            <div class="relative group cursor-pointer" (click)="navigate.emit('/profile')">
              <div class="h-24 w-24 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-lg transition-all group-hover:shadow-indigo-200">
                @if (user()?.avatar) {
            ...
            <div>
              ...
              <h1 class="text-2xl font-black tracking-tight text-slate-900 md:text-3xl">
                Welcome back, <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600">{{ user()?.firstName }}</span>
              </h1>
              <p class="mt-2 max-w-lg text-sm font-medium leading-relaxed text-slate-500">Your command center is ready. Check attendance, requests, holidays, and team updates from one employee workspace.</p>
            </div>
          </div>
          <div class="flex flex-wrap items-center gap-3">
            <button (click)="navigate.emit('/attendance')" class="group relative overflow-hidden rounded-2xl bg-slate-900 px-5 py-3 text-sm font-black text-white hover:shadow-xl hover:shadow-slate-200/50 transition-all">
            ...
            <button (click)="navigate.emit('/leaves')" class="rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-700 transition hover:border-indigo-300 hover:text-indigo-600 hover:shadow-xl">
              Apply Leave
            </button>
            <button (click)="navigate.emit('/profile')" class="rounded-2xl border border-slate-200 bg-slate-50 px-5 py-3 text-sm font-black text-slate-700 transition hover:border-slate-300 hover:bg-white hover:shadow-md">
              View Profile
            </button>
          </div>
        </div>
