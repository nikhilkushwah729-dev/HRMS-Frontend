    ...
    <div class="mx-auto max-w-5xl space-y-6">
      <section class="rounded-md border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div class="flex items-start justify-between gap-4">
          ...
          <a routerLink="/self-service/requests/time-off" class="rounded-md border border-slate-200 px-4 py-3 text-sm font-black text-slate-700 transition hover:bg-slate-50">Back</a>
        </div>
      </section>
        ...
        <article class="rounded-md border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div class="grid gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <input [(ngModel)]="timeOffDate" type="date" class="w-full rounded-md border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-sky-400" />
            </div>
            <div>
              <label class="mb-2 block text-xs font-black uppercase tracking-[0.16em] text-slate-400">Requested Time From</label>
              <input [(ngModel)]="startTime" type="time" class="w-full rounded-md border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-sky-400" />
            </div>
            <div>
              <label class="mb-2 block text-xs font-black uppercase tracking-[0.16em] text-slate-400">Requested Time To</label>
              <input [(ngModel)]="endTime" type="time" class="w-full rounded-md border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-sky-400" />
            </div>
          ...
          <textarea [(ngModel)]="description" rows="6" placeholder="Add supporting details for this time off request" class="mt-4 w-full rounded-md border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-sky-400"></textarea>
          ...
          <div class="mt-5 flex justify-end gap-3">
            <button type="button" (click)="save()" [disabled]="submitting() || !canPersist('draft')" class="rounded-md border border-slate-200 px-5 py-3 text-sm font-black text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">Save Draft</button>
            <button type="button" (click)="submit()" [disabled]="submitting() || !canPersist('pending')" class="rounded-md bg-slate-900 px-5 py-3 text-sm font-black text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50">Submit Time Off</button>
          </div>
        </article>
