      ...
      <section class="overflow-hidden rounded-md border border-slate-200 bg-[radial-gradient(circle_at_top_left,_rgba(15,23,42,0.08),_transparent_38%),linear-gradient(135deg,#ffffff_0%,#f8fafc_55%,#eef2ff_100%)] shadow-sm">
        <div class="grid gap-6 px-4 py-5 sm:px-6 lg:grid-cols-[minmax(0,1fr)_320px] lg:px-8 lg:py-8">
          <div class="space-y-5">
            ...
            <div>
              <h1 class="text-3xl font-black tracking-tight text-slate-900 sm:text-4xl">Shift Policies</h1>
              <p class="mt-3 max-w-2xl text-sm leading-6 text-slate-600">Manage shift timings, grace windows, and roster-ready templates. This page now keeps form controls, status cards, and the shift library aligned on both desktop and mobile.</p>
            </div>

            <div class="grid gap-3 sm:grid-cols-3">
              <div class="rounded-md border border-white/80 bg-white/90 px-4 py-4 shadow-sm">
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Active shifts</p>
                <p class="mt-2 text-2xl font-black text-slate-900">{{ activeCount() }}</p>
                <p class="mt-1 text-xs text-slate-500">Ready for scheduling</p>
              </div>
              <div class="rounded-md border border-white/80 bg-white/90 px-4 py-4 shadow-sm">
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Visible now</p>
                <p class="mt-2 text-2xl font-black text-slate-900">{{ filteredShifts().length }}</p>
                <p class="mt-1 text-xs text-slate-500">Matching current filter</p>
              </div>
              <div class="rounded-md border border-white/80 bg-white/90 px-4 py-4 shadow-sm">
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Mode</p>
                <p class="mt-2 text-lg font-black text-slate-900">{{ editingId() ? 'Edit Shift' : 'Create Shift' }}</p>
                <p class="mt-1 text-xs text-slate-500">Update or add a schedule</p>
              </div>
            </div>
          </div>
          ...
          <div class="rounded-md border border-slate-200 bg-white/90 p-4 shadow-sm sm:p-5">
            <div class="flex items-start justify-between gap-4">
              <div>
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Quick Controls</p>
                <h2 class="mt-1 text-lg font-black text-slate-900">Shift library search</h2>
              </div>
            ...
            <div class="mt-4 space-y-3">
              <label class="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Search shifts</label>
              <div class="relative">
                <input [value]="searchQuery()" (input)="updateSearch($event)" class="w-full rounded-md border border-slate-200 bg-slate-50 py-3 pl-11 pr-4 text-sm text-slate-700 outline-none transition-all focus:border-slate-400 focus:bg-white focus:ring-4 focus:ring-slate-200/60" placeholder="Search by shift name or type">
              ...
              <div class="rounded-md border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                <p class="font-semibold text-slate-900">Navbar controls aligned</p>
                <p class="mt-1 text-xs leading-5 text-slate-500">Search, reset, status cards, and the form panel are now grouped into one stable responsive header.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
        <section class="app-surface-card overflow-hidden p-0">
          <div class="border-b border-slate-100 px-6 py-5">
            ...
            <p class="mt-2 text-sm leading-6 text-slate-600">Set the operational window, shift type, and grace tolerance for your team. The form stacks cleanly on small screens and stays readable on larger layouts.</p>
          </div>
          <div class="px-6 py-6">
            <form [formGroup]="shiftForm" (ngSubmit)="saveShift()" class="space-y-5">
              ...
              <div class="grid gap-3 sm:grid-cols-2">
                <button type="submit" [disabled]="shiftForm.invalid || saving()" class="rounded-md bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:opacity-50">
              ...
              <article class="flex flex-col gap-4 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
                <div class="min-w-0">
                  <div class="flex flex-wrap items-center gap-3">
                    <p class="break-words text-lg font-black text-slate-900">{{ shift.name }}</p>
                  ...
                  <p class="mt-2 break-words text-sm leading-6 text-slate-600">{{ shift.start_time }} to {{ shift.end_time }} | {{ shift.working_hours }} hrs | Grace {{ shift.grace_time }} mins</p>
                </div>
                <div class="flex flex-col gap-3 sm:flex-row">
                  <button type="button" (click)="editShift(shift)" class="rounded-md border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Edit</button>
                  <button type="button" (click)="deleteShift(shift.id)" class="rounded-md border border-rose-200 px-4 py-2.5 text-sm font-semibold text-rose-600 transition hover:bg-rose-50">Delete</button>
                </div>
              </article>
            } @empty {
