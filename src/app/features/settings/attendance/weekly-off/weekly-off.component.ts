    ...
    <div class="mx-auto max-w-7xl space-y-6 px-1 py-2">
      <section class="app-module-hero">
        <div>
          ...
          <p class="app-module-text max-w-2xl">Configure recurring weekly off patterns for business units and keep them persistent even before full backend support is opened for this master.</p>
        </div>
        <div class="app-module-highlight">
          <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Configured policies</p>
          <p class="mt-3 text-3xl font-black text-slate-900">{{ policies().length }}</p>
          <p class="mt-2 text-sm text-slate-600">Active rules: {{ activeCount() }}</p>
        </div>
      </section>
        ...
        <section class="app-surface-card overflow-hidden p-0">
          ...
          <div class="divide-y divide-slate-100">
            @for (policy of filteredPolicies(); track policy.id) {
              ...
              <div class="px-6 py-14 text-center text-slate-500">No weekly off policies found.</div>
            }
          </div>
        </section>
      </div>
...
export class WeeklyOffComponent implements OnInit {
  ...
  private loadPolicies(): void {
    try {
      ...
      if (stored) {
        this.policies.set(JSON.parse(stored));
        return;
      }
    } catch {}
