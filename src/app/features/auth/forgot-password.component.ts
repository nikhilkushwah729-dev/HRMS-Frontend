...
<div class="flex h-[100dvh] flex-col overflow-hidden bg-[radial-gradient(circle_at_top_right,_rgba(20,184,166,0.16),_transparent_22%),radial-gradient(circle_at_bottom_left,_rgba(245,158,11,0.16),_transparent_24%),linear-gradient(135deg,#0f172a_0%,#111827_55%,#0f172a_100%)] lg:flex-row">
  ...
  <div class="hidden lg:flex w-[45%] xl:w-[42%] bg-gradient-to-br from-slate-900 via-primary-900/60 to-slate-900 p-10 xl:p-14 flex-col justify-center relative overflow-hidden text-white flex-shrink-0">
    <div class="relative z-10 max-w-[440px]">
      ...
      <p class="text-sm xl:text-base text-slate-300 leading-relaxed mb-8">{{ t('auth.forgot.heroSubtitle') }}</p>
      <div class="space-y-3">
        <div class="flex items-center gap-3"><div class="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10"/><path d="m9 12 2 2 4-4"/></svg></div><span class="text-sm text-slate-200">{{ t('auth.forgot.featureToken') }}</span></div>
        <div class="flex items-center gap-3"><div class="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg></div><span class="text-sm text-slate-200">{{ t('auth.forgot.featureInstant') }}</span></div>
        <div class="flex items-center gap-3"><div class="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></div><span class="text-sm text-slate-200">{{ t('auth.forgot.featureBank') }}</span></div>
      </div>
    </div>
        ...
        <div class="rounded-[24px] border border-white/10 bg-slate-800/80 p-5 shadow-2xl shadow-black/50 backdrop-blur-xl sm:rounded-[30px] sm:p-7">
          ...
          <form (ngSubmit)="onSubmit()" #forgotForm="ngForm" class="space-y-4">
            <div class="flex flex-col gap-1">
              <label for="email" class="text-xs font-semibold text-slate-300">{{ t('auth.login.workEmail') }}</label>
              <div class="relative">
          ...
          <div class="mt-5 pt-4 border-t border-slate-700 text-center">
            <p class="text-xs text-slate-500">{{ t('auth.forgot.remembered') }} <a routerLink="/auth/login" class="text-primary-400 font-semibold hover:underline">{{ t('auth.signup.signIn') }}</a></p>
          </div>
        </div>
...
export class ForgotPasswordComponent {
  ...
  onSubmit() {
    ...
    if (!this.email) {
      this.error = this.t('auth.login.enterEmail');
      return;
    }
    if (!emailLooksValid) {
      this.error = this.t('auth.login.validEmail');
      return;
    }
    ...
    this.authService.forgotPassword(email).subscribe({
      next: () => {
        this.loading = false;
        this.success = this.t('auth.forgot.success');
      },
      error: (err: any) => {
        this.loading = false;
        this.error = err.error?.message || this.t('auth.forgot.failed');
      }
    });
  }
