...
import { AuditLogService, AuditLogFilter, AuditLog } from '../../core/services/audit-log.service';
...
export class AuditLogsComponent implements OnInit, OnDestroy {
  ...
  private ringToPath(ring: number[][]): string {
    return ring.map((point, i) => `${i === 0 ? 'M' : 'L'}${point[0]} ${-point[1]}`).join(' ') + 'Z';
  }
  ...
  refresh() {
    ...
    this.auditLogService.fetchLogs(this.selectedModule || undefined, this.itemsPerPage, 1, this.getCurrentFilter()).subscribe(() => {
      this.calculateStats();
    });
  }
  ...
  hasActiveFilters(): boolean { 
    return !!(this.searchQuery || this.selectedModule || this.selectedAction || this.startDate || this.endDate || this.selectedQuickDate); 
  }
  
  clearModuleFilter() { this.selectedModule = ''; this.applyFilters(); }
  clearActionFilter() { this.selectedAction = ''; this.applyFilters(); }
  clearDateFilter() { this.startDate = ''; this.endDate = ''; this.selectedQuickDate = ''; this.applyFilters(); }
  ...
  onItemsPerPageChange() { this.applyFilters(); }

  goToPage(page: number) { 
    if (page >= 1 && page <= this.totalPages()) { 
      this.auditLogService.fetchLogs(this.selectedModule || undefined, this.itemsPerPage, page, this.getCurrentFilter()).subscribe(); 
    } 
  }
  ...
  exportSingleLog() {
    ...
    const csv = [headers.join(','), ...rows.map(r => `"${r[0]}","${r[1].replace(/"/g, '""')}"`)].join('\n');
  ...
  getActionClasses(action: string): string {
    ...
    if (a === 'CREATE' || a === 'LOGIN' || a === 'APPROVE' || a === 'REGISTER') return 'bg-green-50 text-emerald-600 border-emerald-600/20';
    if (a === 'UPDATE' || a === 'PASSWORD_CHANGE' || a === 'PASSWORD_RESET') return 'bg-blue-50 text-blue-600 border-blue-600/20';
    if (a === 'DELETE' || a === 'LOGIN_FAILED' || a === 'REJECT') return 'bg-red-50 text-rose-600 border-rose-600/20';
    if (a === 'LOGOUT' || a === 'FORGOT_PASSWORD') return 'bg-amber-50 text-amber-600 border-amber-600/20';
  ...
  getActionIcon(action: string): string {
    ...
    if (a === 'UPDATE' || a === 'PASSWORD_CHANGE' || a === 'PASSWORD_RESET') return '✏️';
