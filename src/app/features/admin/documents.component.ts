        ...
        <div class="p-6 space-y-6">
            ...
            <div class="app-module-hero flex flex-col xl:flex-row xl:items-end xl:justify-between gap-5">
                <div class="max-w-2xl">
                    ...
                    <h1 class="app-module-title mt-3">Employee files and records workspace</h1>
                    <p class="app-module-text mt-3">Manage uploaded records, filter document categories, and handle document lifecycle from one organized repository.</p>
                </div>
                <div class="flex flex-col gap-3 xl:items-end">
                    <div class="app-module-highlight min-w-[240px]">
                        <span class="app-module-highlight-label">Visible files</span>
                        <div class="app-module-highlight-value mt-3">{{ documents.length }}</div>
                        <p class="mt-2 text-sm text-white/80">Documents currently loaded for the selected category and search filters.</p>
                    </div>
            ...
            <div class="bg-white rounded-md shadow-sm border border-gray-200 p-4 mb-6">
                <div class="flex flex-wrap gap-4">
                    ...
                    <div class="flex-1 min-w-[200px]">
                        ...
                        <input type="text" [(ngModel)]="filters.search" (input)="loadDocuments()"
                ...
                <div *ngFor="let doc of documents" 
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="font-medium text-gray-900 truncate max-w-[150px]">{{ doc.name }}</h3>
                                <p class="text-sm text-gray-500">{{ formatFileSize(doc.fileSize) }}</p>
                            </div>
                        </div>
                        <div class="relative">
                            <button (click)="toggleMenu(doc.id)" class="p-1 hover:bg-gray-100 rounded">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-500" viewBox="0 0 20 20" fill="currentColor">
                                    <path d="M10 6a2 2 0 110-4 2 2 0 010 4zM10 12a2 2 0 110-4 2 2 0 010 4zM10 18a2 2 0 110-4 2 2 0 010 4z" />
                                </svg>
                            </button>
                            <div *ngIf="activeMenu === doc.id" class="absolute right-0 mt-1 w-32 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-10">
                                <button (click)="downloadDocument(doc)" class="w-full px-4 py-2 text-left text-sm hover:bg-gray-100">
                                    Download
                                </button>
                                <button (click)="deleteDocument(doc)" class="w-full px-4 py-2 text-left text-sm text-red-600 hover:bg-gray-100">
                                    Delete
                                </button>
                            </div>
                        </div>
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        ...
                        <div *ngIf="!selectedFile" 
                            ...
                            <input #fileInput type="file" (change)="onFileSelect($event)" class="hidden" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png">
                            <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <!-- Selected File -->
                        <div *ngIf="selectedFile" class="border border-gray-200 rounded-lg p-4">
                        ...
                        <!-- Progress Bar -->
                        <div *ngIf="uploading && uploadProgress < 100" class="w-full bg-gray-200 rounded-full h-2.5">
                            <div class="bg-blue-600 h-2.5 rounded-full" [style.width.%]="uploadProgress"></div>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Document Name</label>
                            <input type="text" [(ngModel)]="uploadData.name" 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                                placeholder="Enter document name">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Category</label>
                            <select [(ngModel)]="uploadData.category"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                                <option value="profile">Profile Photo</option>
                                <option value="id_proof">ID Proof</option>
                                <option value="certificate">Certificate</option>
                        ...
                        <div>
                            ...
                            <textarea [(ngModel)]="uploadData.description" rows="2"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
        ...
        return data.filter(item => {
            ...
            if (this.filters.search) {
                ...
                if (!item.name.toLowerCase().includes(search) && 
                    !item.fileName?.toLowerCase().includes(search)) return false;
            }
            return true;
        });
    }
