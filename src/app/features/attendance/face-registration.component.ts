    ...
    <div class="flex flex-col gap-6 pb-10 max-w-4xl mx-auto">
      ...
      >
        <div>
          ...
          <p class="text-slate-500 font-medium mt-1">
            Register your face for touchless attendance marking.
          </p>
        </div>
        <button
          (click)="goBack()"
          class="flex items-center gap-2 px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg font-medium transition-all"
        >
          <svg
      ...
      <div class="bg-blue-50 border border-blue-200 rounded-md p-5">
        ...
        <ul class="space-y-2 text-sm text-blue-800">
          <li class="flex items-start gap-2">
            <span class="font-bold">1.</span> Position your face within the
            camera frame
          </li>
          <li class="flex items-start gap-2">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <div class="p-4">
            ...
            >
              ...
              >
                <div
                  class="w-4/5 h-4/5 border-2 border-primary-500/60 rounded-full relative"
                >
                  <div
                    class="absolute inset-0 bg-primary-500/5 rounded-full animate-pulse"
                  ></div>
                  <div
                    class="absolute -top-1 -left-1 w-6 h-6 border-l-4 border-t-4 border-primary-500 rounded-tl-lg"
                  ></div>
                  <div
                    class="absolute -top-1 -right-1 w-6 h-6 border-r-4 border-t-4 border-primary-500 rounded-tr-lg"
                  ></div>
                  <div
                    class="absolute -bottom-1 -left-1 w-6 h-6 border-l-4 border-b-4 border-primary-500 rounded-bl-lg"
                  ></div>
                  <div
                    class="absolute -bottom-1 -right-1 w-6 h-6 border-r-4 border-b-4 border-primary-500 rounded-br-lg"
                  ></div>
                </div>
              </div>
              ...
              <div
                *ngIf="isProcessing()"
                class="absolute inset-0 bg-slate-900/70 backdrop-blur-sm flex flex-col items-center justify-center gap-3"
              >
                <svg
        ...
        <div class="flex flex-col gap-4">
          <!-- Status Card -->
          <div
            class="card p-5 rounded-md border"
            [ngClass]="
              isFaceRegistered()
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <h4
                  class="font-bold"
                  [ngClass]="
                    isFaceRegistered() ? 'text-green-900' : 'text-amber-900'
                  "
                >
                  {{
                    isFaceRegistered()
                ...
                <p
                  class="text-sm"
                  [ngClass]="
                    isFaceRegistered() ? 'text-green-700' : 'text-amber-700'
                  "
                >
                  {{
                    isFaceRegistered()
                      ? 'You can mark attendance using face recognition'
          ...
          <div class="bg-slate-50 rounded-md p-4">
            <h4
              class="font-bold text-slate-700 text-sm mb-2 flex items-center gap-2"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left">
            <thead class="bg-slate-50">
              <tr>
                <th
                  class="px-4 py-3 text-xs font-bold text-slate-500 uppercase"
                >
                  Employee
                </th>
                <th
            ...
            <tbody class="divide-y divide-slate-100">
              ...
              <tr
                *ngFor="let emp of registeredEmployees()"
                class="hover:bg-slate-50/50"
              >
                <td class="px-4 py-3">
                  <div class="flex items-center gap-3">
                    <div
                      class="w-8 h-8 rounded-full bg-primary-100 flex items-center justify-center text-primary-700 font-bold text-xs"
                    >
                ...
                <td class="px-4 py-3 text-sm text-slate-600">
                  {{ emp.registeredAt | date: 'MMM dd, yyyy' }}
                </td>
                <td class="px-4 py-3">
                  <span
                    class="px-2 py-1 bg-green-50 text-green-700 text-xs font-bold rounded-md"
                  >
                    Active
                  </span>
                </td>
                <td class="px-4 py-3">
                  <button
                    *ngIf="emp.employeeId === currentUser?.id"
                    (click)="deleteRegistration(emp.employeeId)"
...
export class FaceRegistrationComponent implements OnInit, OnDestroy {
  ...
  checkFaceRegistration() {
    ...
    this.faceRecognitionService
      .hasRegisteredFace(this.currentUser.id)
      .subscribe({
        next: (hasFace) => {
          ...
          next: (res) => {
            ...
            if (res.success) {
              this.toastService.success(
                `Welcome ${res.employee?.name}! Attendance marked.`,
              );
